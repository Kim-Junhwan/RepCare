...
||||||
|이터레이션5|||||~ 2023/10/11|
||종 선택 화면|종 선택시 하위 종 가져오는 기능 구현|2h|4h|6|
||종 선택 화면|종선택 섹션 마지막 셀 등록셀로 구현|2h|1h|6|
||종 선택 화면|ViewModel 바인딩, 선택된 종이 빈 하위 종을 가져올 경우 등록셀이 안나오던 버그 수정|2h|1h|6|
||펫 등록 화면|텍스트필드 툴바 구현|2h|1h|6|
|||Presentation 계층에서 사용할 Model 구현|2h|1h 30m|6|
||펫 등록 화면|입양일, 해칭일 날짜 선택시 DatePicker ActionSheet 나오게 구현|2h|1h 30m|6|
||펫 등록 화면|ViewModel 구현 및 바인딩|2h|1h|6|
||펫 등록 화면|상위 펫 선택시, 선택되었던 하위 종은 사라지는 기능 구현|2h|1h|6|
||||||
|이터레이션6|||||~ 2023/10/15|
||종 선택 화면|종 추가 셀 클릭 시 textField Alert 나오게 설정|1h|1h|4|
||펫 등록 화면|종 선택화면에서 적용 버튼 누를 시, 펫 등록 화면에서 선택된 종 표시|1h|1h|4|
||종 선택 화면|종 추가시 추가한 종 바로 가져오지 못하던 버그 수정|1h|1h|4|
||종 선택 화면|종 가나다순 정렬해서 가져오기|1h|1h|4|
||펫 등록 화면|펫 이미지 선택 셀, 이미지 셀 UI 구현|1h|1h|4|
||펫 등록 화면|카메라, 앨범에서 이미지 등록 구현|1h|1h|4|
||Domain|펫 등록 usecase 정의|30m|30m|4|
||펫 등록 화면|펫 등록시 이미지 다운샘플링 및 FileManager로 저장|1h|1h|4|
||펫 등록 화면|펫 등록시 이미지 다운샘플링 및 FileManager로 저장|1h|1h|4|
||펫 리스트 화면|펫 리스트 화면에서 저장된 사진이 있다면 가장 첫번째 사진이 나오도록, 사진이 없는 경우 종 이미지를 대신 보여주도록 구현|1h|1h|4|
||펫 리스트 화면|펫 리스트 화면에서 종 선택시 종 필터링|1h|1h|4|
||||||
|이터레이션7|||||~ 2023/10/18|
||펫 상세 화면|프로필 화면 탭바 NestedScrollView 구현|2h|30h|16|
||펫 상세 화면|펫 캘린더 ViewController 구현|2h|30h|16|
||펫 상세 화면|팻 캘린더 로직 구현|2h|30h|16|
||펫 상세 화면|펫 캘린더 ViewController 작업 셀 클릭시 캘린더에 이벤트 추가 및 하단 타임라인데 추가|2h|30h|16|
||펫 상세 화면|펫 상세화면 header ViewController 구현|2h|30h|16|
||||||
|이터레이션8|||||~ 2023/10/22|
||펫 상세 화면|펫 무게 차트 구현|2h|30h|16|
||펫 상세 화면|펫 무게 ViewController 구현|2h|30h|16|
||펫 상세 화면|펫 무게 화면에서 무게 추가시 차트에 추가|2h|30h|16|
||펫 상세 화면|펫 무게 화면에서 삭제 기능 구현|2h|30h|16|
||펫 상세 화면|펫 캘린더 화면 버그 수정|2h|30h|16|
||||||
|이터레이션9|||||~ 2023/10/25|
|||Calendar객체가 필요할때마다 생성했던 코드 제거|2h|30h|16|
||펫 등록 화면|이미지 저장시 이미지가 회전되던 버그 수정|1h|1h|4|
||펫 리스트 화면|검색 기능 구현|1h|1h|4|
||종 선택 화면|종 선택화면 UI 변경|1h|1h|4|
|이터레이션10|||||~ 2023/10/30|
||펫 상세 화면|펫 삭제 기능 구현|2h|30h|16|
||펫 상세 화면|펫 정보 수정버튼 클릭시, 현재 펫 정보 수정 화면에 바인딩|2h|30h|16|
||종 선택 화면|펫 등록화면에서 기존에 선택된 종이 있다면, 종이 선택된 채로 나오는 기능 구현|1h|1h|4|
||펫 상세 화면|펫 정보 수정버튼시 헤더 뷰 정보 업데이트|2h|30h|16|
